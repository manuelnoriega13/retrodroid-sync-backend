...
func main() {
	...
	dirPath := flag.String("dir", "", "Directorio base donde se guardarán los saves de los emuladores (REQUERIDO)")
	...
	storage := upload.NewDiskStorage(*dirPath)
	service := upload.NewService(storage)
	handler := upload.NewHandler(service)

	// 3. Registrar rutas
	http.HandleFunc("/upload", handler.ServeHTTP)
